...
class Api:
    ...
    def save_image(self, b64_data, suffix, ext, path_index, custom_text):
        # 1. 处理前端传来的命名文字（彻底删除所有空格和非法字符）
        clip_text = custom_text.strip()
        if clip_text:
            clip_text = re.sub(r'\s+', '', clip_text) # 删掉所有形式的空格/换行
            clip_text = re.sub(r'[\\/*?:"<>|]', '', clip_text) # 过滤非法系统字符
            clip_text = clip_text[:100] # 最长 100 字防报错
...
<script>
    ...
    window.addEventListener('pywebviewready', function() {
        ...
        // 自动监听剪贴板（如果内容改变且输入框没被聚焦打字，则自动填入）
        setInterval(() => {
    ...
    function saveLongImage(pathIndex, autoClear) {
        ...
        pywebview.api.save_image(b64, '', '.jpg', pathIndex, customName).then(msg => {
            showToast(msg);
            if (autoClear) {
                images = []; 
                saveHistory();  // 记录下当前“空”的状态
                renderThumbnails(); 
    ...
    document.getElementById('saveSingleBtn').addEventListener('click', () => {
        ...
        images.forEach((img, idx) => {
            ...
            pywebview.api.save_image(b64, `_片段${idx+1}`, '.png', 1, customName).then(msg => { console.log(msg); });
        });
    });
    ...
    function focusEdge(edge) {
        ...
        if (!selectedBox) {
            if (imageHitBoxes.length > 0) {
                selectedBox = imageHitBoxes[0];
            } else {
                showToast("没有可定位的图片");
                return;
            }
        ...
        // 纵向按边缘平移
        if (edge === 'top') {
            transformMatrix.y = 20 - selectedBox.y0 * targetScale; // 顶部并预留 20px 边距
        } else if (edge === 'bottom') {
            transformMatrix.y = rect.height - 20 - selectedBox.y1 * targetScale; // 底部预留 20px 边距
        }
        updateCanvasDisplay();
        updateBrushCursor();
    }
    ...
    function resetView(isInitial = false) { 
        ...
        if (isInitial && images.length > 0) {
            ...
            let sw = Math.max(1, firstImg.imgObj.width - firstImg.crop.left - firstImg.crop.right);
            let sh = Math.max(1, firstImg.imgObj.height - firstImg.crop.top - firstImg.crop.bottom);
            let scaleToMaxW = canvasSize.width / sw;
            let firstImgRenderHeight = sh * scaleToMaxW;
            
            // 目标：让 1.5 张图的高度刚好占满视口高度的 85%
            ...
            // 两者取小，确保不管是长条图还是宽方图，都能优雅地显示出 1.5 张
    ...
    function uploadFiles(files, source = 'drag') {
        ...
        Promise.all(promises).then(newImgs => {
            ...
            // 按照需求进行不同视角的默认渲染
            if (source === 'drag') {
                queueRenderPreview('all'); // 拖拽 -> 默认显示全部
            } else {
                if (files.length > 1) {
                    queueRenderPreview('third'); // 批量点击上传 -> 显示预览区的三分之一
                } else {
                    queueRenderPreview('all');
                }
            }
            
            showToast(`导入 ${newImgs.length} 张图片`);
        });
    }
    
    // 分别传递 'click' 或 'drag' 源头给 uploadFiles
